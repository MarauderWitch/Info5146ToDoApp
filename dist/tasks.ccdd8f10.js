...
document.addEventListener("DOMContentLoaded", function() {
    ...
    const aiButton1 = document.getElementById('send-btn');
    const aiInput1 = document.getElementById('chat-input');
    const chatHistory1 = document.getElementById('chat-history');
    const signOutBttn1 = document.getElementById("signOutBttn");
    if (!taskInput1 || !addTaskBtn1 || !taskList1 || !aiButton1 || !aiInput1 || !chatHistory1 || !signOutBttn1) {
        console.error("One or more elements not found! Check your HTML.");
        return;
    }
    // ✅ Now add event listeners safely
    addTaskBtn1.addEventListener('click', async ()=>{
    ...
    aiButton1.addEventListener('click', async ()=>{
        let prompt = aiInput1.value.trim().toLowerCase();
        if (prompt) console.log("Chatbot received: ${prompt}");
        else appendMessage("Please enter a prompt");
    });
    signOutBttn1.addEventListener("click", function() {
...
async function addTaskToFirestore(taskText) {
    let task = await (0, _firestore.addDoc)((0, _firestore.collection)(db, "todos"), {
        text: taskText,
        email: email,
        completed: false
    });
...
async function getTasksFromFirestore() {
    let q = (0, _firestore.query)((0, _firestore.collection)(db, "todos"), (0, _firestore.where)("email", "==", email));
...
addTaskBtn.addEventListener('click', async ()=>{
    ...
    if (taskText) {
        (0, _loglevelDefault.default).info("User clicked 'Add Task' with input: ${taskText}");
        await addTaskToFirestore(taskText);
        taskInput.value = "";
    } else {
        (0, _loglevelDefault.default).warn("User tried to add an empty task");
        alert("Please enter a task!"); // ✅ Show an alert for empty input
    }
});
// ✅ Add Task on Enter Key Press
taskInput.addEventListener("keypress", function(event) {
    if (event.key === "Enter") addTaskBtn.click(); // Simulate button click
});
...
taskList.addEventListener("keypress", async function(e) {
    if (e.target.tagName === 'LI' && e.key === "Enter") try {
        (0, _loglevelDefault.default).info("Marking task as completed: ${e.target.id}");
        await (0, _firestore.updateDoc)((0, _firestore.doc)(db, "todos", e.target.id), {
            completed: true
        });
        (0, _loglevelDefault.default).info("Task ${e.target.id} marked as completed");
...
if ('serviceWorker' in navigator) navigator.serviceWorker.register(sw.href, {
    scope: '/Info5146ToDoApp/'
}).then(()=>(0, _loglevelDefault.default).info('Service Worker Registered successfully'))//then(_
.catch((err)=>(0, _loglevelDefault.default).error('Service Worker Error:', err));
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"4b8AD":[function(require,module,exports,__globalThis) {
module.exports = require("3a7d656bb8fdd828").getBundleURL('aaA9g') + "service-worker.06934e42.js" + "?" + Date.now();

},{"3a7d656bb8fdd828":"lgJ39"}],"lgJ39":[function(require,module,exports,__globalThis) {
"use strict";
var bundleURL = {};
...
},{}]},["3lXTp","azFBe"], "azFBe", "parcelRequire94c2")
